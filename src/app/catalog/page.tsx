    ...
    <div className="bg-surface text-on-surface">
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#f7f6f3]/70 dark:bg-[#1a1a19]/70 backdrop-blur-md shadow-[0_12px_40px_rgba(146,63,95,0.08)] h-20 flex justify-between items-center px-8 font-plus-jakarta tracking-tight">
        ...
        <div className="flex items-center gap-6">
          <button className="text-[#5b5c5a] hover:scale-105 transition-transform duration-200 flex items-center" onClick={() => alert("Carrito vacío")}>
            <span className="material-symbols-outlined">shopping_cart</span>
          </button>
          <button className="text-[#5b5c5a] hover:scale-105 transition-transform duration-200 flex items-center" onClick={() => window.location.href = "/admin"}>
      ...
      <main className="pt-20">
        ...
        <section className="relative px-8 py-16 md:py-24 overflow-hidden bg-surface-container-low">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12 relative z-10">
            ...
            <div className="flex-1 relative">
              ...
              <img 
                className="relative z-10 w-full h-[500px] object-cover rounded-xl shadow-[0_20px_60px_rgba(146,63,95,0.12)] -rotate-2 hover:rotate-0 transition-transform duration-500" 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
            {loading ? (
              Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="animate-pulse bg-surface-container-lowest rounded-xl p-4 h-[400px]">
                  <div className="bg-surface-container-high rounded-lg w-full aspect-square mb-6"></div>
                ...
                return (
                  <div key={product.id} className="group relative flex flex-col bg-surface-container-lowest rounded-xl p-4 shadow-[0_12px_40px_rgba(146,63,95,0.08)] hover:scale-[1.02] transition-all duration-300">
                    <div className="relative w-full aspect-square overflow-hidden rounded-lg mb-6">
                      <img className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" src={primaryImage} alt={product.name} />
                      {product.inventory?.status && (
                        <span className={`absolute top-4 left-4 px-3 py-1 rounded-full text-[10px] font-bold uppercase transition-all ${
                    ...
                    <div className="flex flex-col flex-grow">
                      ...
                      <div className="mt-auto flex items-center justify-between">
                        ...
                        <button className="w-12 h-12 rounded-full bg-surface-container-low flex items-center justify-center text-primary hover:bg-primary hover:text-on-primary transition-colors" onClick={() => alert(`${product.name} añadido al carrito`)}>
                          <span className="material-symbols-outlined">shopping_cart</span>
                        </button>
                      </div>
                    </div>
        ...
        <section className="px-8 py-20">
          <div className="max-w-5xl mx-auto rounded-xl bg-primary-container/10 p-12 relative overflow-hidden flex flex-col items-center text-center">
            ...
            <form className="flex flex-col md:flex-row w-full max-w-md gap-4" onSubmit={(e) => { e.preventDefault(); alert('Suscrito!'); }}>
