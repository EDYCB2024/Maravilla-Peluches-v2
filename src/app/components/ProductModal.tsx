...
export default function ProductModal({ product, isOpen, onClose, dollarRate = 0 }: ProductModalProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6 md:p-8 transition-all duration-300 ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
    >
      {/* Backdrop */}
      <div
      ...
      {/* Modal Content */}
      <div
        className={`relative w-full max-w-4xl max-h-[85vh] bg-surface rounded-[2rem] overflow-hidden shadow-[0_24px_70px_rgba(146,63,95,0.2)] flex flex-col md:flex-row transition-all duration-500 transform ${isOpen ? "scale-100 translate-y-0" : "scale-95 translate-y-8"
          }`}
      >
        {/* Close Button */}
        <button
